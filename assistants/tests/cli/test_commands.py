...
def test_copies_response_to_clipboard(mock_inform, mock_copy):
    environ = IoEnviron(assistant=MagicMock(), last_message=MagicMock(text_content="Response text"))
...
@patch("assistants.cli.commands.pyperclip.copy", side_effect=pyperclip.PyperclipException)
@patch("assistants.cli.commands.output.fail")
def test_handles_clipboard_copy_error(mock_fail, mock_copy):
    environ = IoEnviron(assistant=MagicMock(), last_message=MagicMock(text_content="Response text"))
    command = CopyResponse()
    asyncio.run(command(environ))
    mock_fail.assert_called_once_with("Error copying to clipboard; this feature doesn't seem to be available in the current terminal environment.")
...
@patch("assistants.cli.commands.pyperclip.copy")
@patch("assistants.cli.commands.output.inform")
def test_copies_code_blocks_to_clipboard_with_lang_and_single_newline(mock_inform, mock_copy):
    environ = IoEnviron(assistant=MagicMock(), last_message=MagicMock(text_content="```lang\ncode block```"))
...
def test_copies_code_blocks_to_clipboard_with_newline_only(mock_inform, mock_copy):
    environ = IoEnviron(assistant=MagicMock(), last_message=MagicMock(text_content="```\ncode block```"))
    command = CopyCodeBlocks()
    asyncio.run(command(environ))
    mock_copy.assert_called_once_with("code block")
    mock_inform.assert_called_once_with("Copied code block to clipboard")
...
def test_copies_code_blocks_without_newlines_to_clipboard(mock_inform, mock_copy):
    environ = IoEnviron(assistant=MagicMock(), last_message=MagicMock(text_content="```code block```"))
    command = CopyCodeBlocks()
    asyncio.run(command(environ))
    mock_copy.assert_called_once_with("code block")
    mock_inform.assert_called_once_with("Copied code block to clipboard")
...
def test_warns_when_no_code_blocks_to_copy(mock_warn):
    environ = IoEnviron(assistant=MagicMock(), last_message=MagicMock(text_content="No code blocks here"))
    command = CopyCodeBlocks()
    asyncio.run(command(environ))
